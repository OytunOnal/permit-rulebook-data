...
| EU Blue Card — whether the reduced threshold carries an institution restriction (2026-09-07) | **Unverified, and deliberately not guessed at.** s5d replaced the country-less `qualification_recent` gate on the three Dutch routes because the IND's own text puts an institution restriction behind it. The Spanish limb we model reads "una cualificación… obtenida en los últimos 3 años" and states no institution scope; the source is the PDF tier, so nobody here can read it either way. `es-blue-card` therefore keeps `qualification_recent` unchanged, and the criterion carries a note saying so. Needs a human read of Orden PJC/44/2026. |
