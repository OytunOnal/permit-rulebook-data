...
| EU Blue Card — whether the reduced threshold carries an institution restriction (2026-09-07) | **Unverified, and deliberately not guessed at.** s5d replaced the country-less `qualification_recent` gate on the three Dutch routes because the IND's own text puts an institution restriction behind it. The Spanish limb we model reads "una cualificación… obtenida en los últimos 3 años" and states no institution scope; the source is the PDF tier, so nobody here can read it either way. `es-blue-card` therefore keeps `qualification_recent` unchanged, and the card says so to the reader in its own words: the reading `salary-figures-and-their-limits-read-by-a-person`. The criterion carried an unsourced `note` saying the same thing until 2026-09-07, when `note` was removed as the one renderable slot that declared no kind (review). Needs a human read of Orden PJC/44/2026. |
